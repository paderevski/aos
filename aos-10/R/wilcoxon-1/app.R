  ...
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput("alpha_level", 
                   ...
                   0.05),
      actionButton("ttest_one_sided_button", "Perform One-Sided Test"),
      actionButton("ttest_two_sided_button", "Perform Two-Sided Test")
    ),
    mainPanel(
...
server <- function(input, output) {
  ...
  perform_ttest <- function(one_sided = FALSE) {
    ...
    result_test <- wilcox.test(num_vector, mu = hypothesized_mean, alternative = alternative, conf.level = 1-alpha)
    
    # Display the t-test results
    output$ttest_result <- renderTable({
      results <- data.frame(
        Description = c("Degrees of Freedom", "P-Value", "Sample Mean", "Hypothesized Mean", "T-Test Result"),
        Value = c(result_test$statistic, result_test$parameter, result_test$p.value, 
                   mean(num_vector), 
                  hypothesized_mean, 
                  ifelse(result_test$p.value < alpha, "Reject Null Hypothesis: Difference is significant", 
