  ...
  moduleServer(id, function(input, output, session) {
    ...
    perform_ttest <- function(one_sided = FALSE) {
      ...
      if (length(num_vector_x) != length(num_vector_y)) {
        output$ttest_result <- renderPrint({ "Unequal vector lengths. Unable to perform paired test." })
        return()
      }
      ...
      t_test <- t.test(num_vector_x, num_vector_y,alternative = alternative, conf.level = 1-alpha, na.action = na.omit, paired=TRUE)
