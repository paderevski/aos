...
2. If $f(x)=x+\dfrac{1}{x}$, then the set of values for which $f$ increases is
...
3. If $\mathrm{f}(\mathrm{x})=\dfrac{\ln x}{\mathrm{x}}$, for all $\mathrm{x}>0$, which of the following is true?
...
5. The graph of $y=\dfrac{-5}{x-2}$ is concave downward for all values of $x$ such that
